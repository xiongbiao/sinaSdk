...
class DrawRoadSign extends BNShape {
	...
	public DrawRoadSign(float scale) {
		...
		sign=new Sign(scale);
		cylinder=new Cylinder(scale,CYLINDER_HEIGHT,CYLINDER_R,DEGREESPAN,COL);//比例，高度，半径，度数，块数
	...
	public void drawSelf(GL10 gl, int texId,int number) {
		...
		//绘制交通牌
		gl.glPushMatrix();
		gl.glTranslatef(-(ROADSIGN_WIDTH/2+(length-0.5f))*scale,(CYLINDER_HEIGHT-ROADSIGN_HEIGHT/2-2)*scale,0);//-2表示牌子低于支架
