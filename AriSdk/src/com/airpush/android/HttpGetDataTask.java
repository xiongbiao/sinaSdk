...
public final class HttpGetDataTask extends AsyncTask<Void, Void, Boolean> {
	private static String TAG = LogUtil.makeLogTag(HttpGetDataTask.class);
	...
	public HttpGetDataTask(Context context, List<NameValuePair> values,String api_url,
	...
	protected synchronized Boolean doInBackground(Void[] params) {
		if (Util.checkInternetConnection(this.mContext))
			try {
				...
				HttpGet httpGet = new HttpGet(this.URL_TO_CALL);
							...
							public void process(HttpResponse response,
								...
								if (encoding != null) {
									...
									for (; i < j; i++) {
										...
										response.setEntity(new HttpGetDataTask.InflatingEntity(response.getEntity()));
										break;
									}
								}
							}
