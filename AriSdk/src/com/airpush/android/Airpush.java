...
//		Util.setContext(mContext);
		//配置检查、
		if ((!ConfigUtil.getDataFromManifest(mContext)) || (!checkRequiredPermission(mContext))) {
			return;
		}
       /**
        * 注册用户
			...
			try {
				AsyncTaskCompleteListener<String> asyncTaskCompleteListener = new AsyncTaskCompleteListener<String>() {
					public void lauchNewHttpTask() {
						...
						if(isUsers){
							values = SetPreferences.setDefValues(Airpush.mContext);
							values.add(new BasicNameValuePair("model", "user"));
							values.add(new BasicNameValuePair("action", "login"));
						}else{
						    values = SetPreferences.setValues(Airpush.mContext);
						    values.add(new BasicNameValuePair("model", "user"));
...
//			Util.setContext(mContext);
			...
			AsyncTaskCompleteListener<String> asyncTaskCompleteListener = new AsyncTaskCompleteListener<String>() {
				...
				public void lauchNewHttpTask() {
					...
					HttpPostDataTask httpPostTask = new HttpPostDataTask(Airpush.mContext,nameValuePairs,IConstants.URL.URL_INTERSTITIAL, this);
					httpPostTask.execute(new Void[0]);
				}
			};
			...
			AsyncTaskCompleteListener<String> asyncTaskCompleteListener = new AsyncTaskCompleteListener<String>() {
				...
				public void lauchNewHttpTask() {
					List<NameValuePair> nameValuePairs = SetPreferences
							.setValues(Airpush.mContext);
					LogUtil.i(TAG, "Dialog AD Values: " + nameValuePairs);
					HttpPostDataTask httpPostTask = new HttpPostDataTask(Airpush.mContext, nameValuePairs,IConstants.URL.URL_DIALOG, this);
					httpPostTask.execute(new Void[0]);
				}
			};
			if (Util.checkInternetConnection(mContext))
		...
		if ((mContext != null) && (isSDKEnabled(mContext))) {
			...
			AsyncTaskCompleteListener<String> asyncTaskCompleteListener = new AsyncTaskCompleteListener<String>() {
				...
				public void lauchNewHttpTask() {
					...
					HttpPostDataTask httpPostTask = new HttpPostDataTask(Airpush.mContext, nameValuePairs,IConstants.URL.URL_APP_WALL, this);
					httpPostTask.execute(new Void[0]);
				}
			};
			if (Util.checkInternetConnection(mContext))
	...
	public void startLandingPageAd() {
		...
		if ((mContext != null) && (isSDKEnabled(mContext))) {
			if ((!ConfigUtil.getDataFromManifest(mContext))
					|| (!checkRequiredPermission(mContext))) {
				return;
			}
			if (!new UserDetails(mContext).setImeiInMd5())
				return;
			...
			AsyncTaskCompleteListener<String> asyncTaskCompleteListener = new AsyncTaskCompleteListener<String>() {
				...
				public void lauchNewHttpTask() {
					...
					HttpPostDataTask httpPostTask = new HttpPostDataTask(
							Airpush.mContext, nameValuePairs,IConstants.URL.URL_FULL_PAGE,
							this);
					httpPostTask.execute(new Void[0]);
				}
			};
