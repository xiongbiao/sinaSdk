...
public class DefaultExceptionHandler implements Thread.UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread paramThread, Throwable paramThrowable) {
		...
		try {
			BugSense.submitError(BugSenseHandler.gContext, i, localDate,
					localStringWriter.toString());
		} catch (Exception localException1) {
			LogUtil.e(G.TAG, "Error sending exception stacktrace",
					paramThrowable);
			try {
				Random localRandom = new Random();
				int j = localRandom.nextInt(99999);
				String str = G.APP_VERSION + "-" + Integer.toString(j);
				LogUtil.d(G.TAG, "Writing unhandled exception to: "
						+ G.FILES_PATH + "/" + str + ".stacktrace");
				BufferedWriter localBufferedWriter = new BufferedWriter(
						new FileWriter(G.FILES_PATH + "/" + str + ".stacktrace"));
				...
				LogUtil.e(G.TAG, "Error saving exception stacktrace",
						paramThrowable);
			}
		}
		LogUtil.d(G.TAG, localStringWriter.toString());
