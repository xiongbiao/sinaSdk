...
public class FullView extends RelativeLayout {
	...
	public FullView(Context context, AttributeSet attrs, int defStyle) {
	...
	public FullView(Context context , AdR adR) {
		super(context);
		// TODO Auto-generated constructor stub
		mContext = context;
	...
	private void initView(){
        ...
        rParams = new RelativeLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT);
	...
	private void addCloseView(){
		try {
			...
			rParams = new RelativeLayout.LayoutParams(dip2px(mContext, 30), dip2px(mContext, 30));
	        rParams.addRule(RelativeLayout.ALIGN_PARENT_TOP, RelativeLayout.TRUE);
	        rParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT, RelativeLayout.TRUE);
	        rParams.topMargin = dip2px(mContext, 8);
	        rParams.rightMargin = dip2px(mContext, 8);
	        addView(mTopRightButton, rParams);
		} catch (Exception e) {
	...
	private void addInfoImageView(){
		try {
			...
			rParams = new RelativeLayout.LayoutParams(LayoutParams.FILL_PARENT, dip2px(mContext, height/5));
...
//	        addView(mInfoView, rParams);
	        ...
	        rParams = new RelativeLayout.LayoutParams(dip2px(mContext, width*8/9), dip2px(mContext, height/7));
//	        rParams.addRule(RelativeLayout.CENTER_IN_PARENT , RelativeLayout.TRUE);
	        rParams.topMargin = dip2px(mContext, height*1/4);
	        rParams.leftMargin = dip2px(mContext, width/18);
	        rParams.rightMargin = dip2px(mContext, width/18);
	        
	        mInfoView = new LinearLayout(mContext);
	        mInfoView.setPadding(5, 5, 5, 5);
	        mInfoView.setOrientation(LinearLayout.HORIZONTAL);
			...
			LinearLayout.LayoutParams xingParams = new LinearLayout.LayoutParams(dip2px(mContext,80),dip2px(mContext,15));
	        ...
	        xingParams.gravity = LEFT_OF;
	        xingParams.leftMargin = dip2px(mContext,10);
	...
	private void infoTvView(){
		...
		rParams = new RelativeLayout.LayoutParams(dip2px(mContext, width*8/9), dip2px(mContext, width/4));
		rParams.topMargin = dip2px(mContext, height*19/51);
	    rParams.leftMargin = dip2px(mContext, width/18);
	    rParams.rightMargin = dip2px(mContext, width/18);
