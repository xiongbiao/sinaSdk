...
public class RandomStr {
	...
	if (upper.size() == 3) 
	{ 
	// check for the number of ranges max 3 uppercase lowercase digits build the
	// random string
	for (int i = 0; i < length.intValue(); i++) 
	{ 
	// you have four groups to choose a random number from, to make
	// the choice a little more random select a number out of 100 get a random
	// number
...
//	even or odd 
	...
	// two different ranges build the random char from single chars and two
	// ranges
	for (int i = 0; i < length.intValue(); i++) 
	{ 
