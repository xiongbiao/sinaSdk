...
import com.kkpush.app.controller.AppSettingController;
...
public class SendMail {
	...
	// return true;
	// } catch (Exception e) {
	// logger.debug("增加邮件附件：" + filename + "发生错误！" + e);
	// return false;
	// }
	// }
	...
	public static void fromMail(Mailinfo mailInfo) throws Exception {
		try {
			  ...
			  String smtp = SystemConfig.getProperty("mail.smtp", "");
			  String username = SystemConfig.getProperty("mail.username", "");
			  String pwd = SystemConfig.getProperty("mail.passWord", "");
			
			  SendMail themail = new SendMail(smtp);
			  themail.setNamePass(username, pwd);//发送者的邮箱名，密码
			  themail.setNeedAuth(true);//是否要验证

			  logger.debug(smtp+"--"+username+"---"+pwd);
	...
	public static void main(String[] args) throws Exception {
		 ...
		 try {
			 Mailinfo mailInfo =new Mailinfo();
				logger.debug("2--------");
				String toMail = SystemConfig.getProperty("mail.tomail", "");
				mailInfo.setToMail(toMail);
//				mailInfo.setMailbody("广告ID :   广告json详情 :  <br/>" );
				 String  con =	PublicController.getEmail(SystemConfig.getProperty("httpUrl", ""),"sss","sdasas@qq.com");
