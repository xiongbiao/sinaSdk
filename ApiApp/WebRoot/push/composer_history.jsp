			...
			<div id="content">
				...
				<div class="right_content" style="width: 72%;">
					...
					<table class="list">
						<thead>
							<tr>
								<th id="test" style="width: 15%;"><span>发送时间</span>
								</th>
								<th id="test"><span>消息内容</span>
								</th>
	...
	<script type="text/javascript">
		   $(function(){
			function getErrmsg(code) {
		      ...
		      //加载消息通知列表pageIndex	1
		     jQuery.post("${ctx}/pushMsg/getPushList",{"pageIndex" : 1,"msgType":1,"appId":<%=appId %>,"isScheduled": false},  function(data) {
			     var str="";
			     ...
			     if (data.rows.length > 0) {
				     for(var i=0;i<data.rows.length;i++){
				        ...
				        str = str+ ' <tr>' +
									...
									'	<td id = "td_type"><span msg= '+rv +'>' + typeStr + showrv+'</span></td>' +
									'   <td>'+getErrmsg(data.rows[i].errorCode)+'</td>'+
								    '   <td>'+totalUserStr+'</td>'+
                                    '   <td>'+sendCntStr+'</td>'+
                                    '   <td>'+(data.rows[i].sendSource?'api':'portal')+'</td>'+
									'	<td> <a href="${ctx}/push/newMsg/<%=appId%>/'+data.rows[i].msgId+'.html">复制</a>'+
