							...
							public void run() {
								...
								if (stageNo < 5) {
									if (helpRole == null) {
										...
										helpRole.setLocation(LLKScreen.this
												.getWidth()
		...
		for (int y = 0; y < yBound; y++) {
			for (int x = 0; x < xBound; x++) {
				...
				if (x == 0 || x == xBound - 1 || y == 0 || y == yBound - 1) {
					LImage img = Images.getInstance().getImage(count % sub);
					int nx = offsetX + x * img.getWidth();
					int ny = offsetY + y * img.getHeight();

					grid[y][x].setLocation(nx, ny);
					grid[y][x].setImage(Images.getInstance().getImage(9));
